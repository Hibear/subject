...
if (! function_exists('css_js_url')){
    function css_js_url($css_js_uri, $app_type,$v2 = ""){

        if(empty($v2)){
			 $static_url = C('domain.static.url');
		}else{
			 $static_url = C('domain.www_static.url');
		}
        ...
        $version = C('css_js_version')["www"][$type];
        //从数据库中查询版本号
 ...
 * 获取static的img
 *
 * @param string $static_img_uri
 *
 * @return string $img_uri
 * @765862579@qq.com
 */
 if (! function_exists('static_img_uri')){
    function static_img_uri($img_path = ''){
        return C('domain.www_static.url') . '/'.$img_path;
    }
}
...
if (! function_exists('css_js_url_v2')){
    function css_js_url_v2($css_js_uri, $app_type) {
       echo  css_js_url($css_js_uri, $app_type,2);
     }
}
