$(function(){
    ...
    $('.btn-search').click(function(){
        var params = { media_type: $("input[name='order_type']").val(), point_status: $("#point_status").val() };
        if ($("#point_status").val() == 2) params['customer_id'] = $("select[name='customer_id']").val();
        if($("#media_id").val()) params['media_id'] = $("#media_id").val();

        $.post('/orders/get_points', params, function(data){
            $("#points_lists").empty();
            $("select[name='make_num']").val('2');
            if (data.flag && data.points_lists.length > 0) {
                ...
                for (var i = 0; i < lists.length; i++) {
                    ...
                    if (order_type == 1 || order_type == 2) {
                        html += '<td class="col-sm-2"><input type="text" style="width:91px" value="2" /></td>';
                    }
