$(function(){
    ...
    $(".mutile_unlock").click(function(){
        ...
        $('input[name="checkbox"]:checked').each(function(){
            ...
            chk_code.push($(this).attr("data-code"));
        });
        if(!is_free){
            artDialog("信息提示","你选择的点位里含不是锁定状态的！");
            return false;
        }
        if (chk_value.length == 0) {
            artDialog("信息提示","你选择你要解锁的点位");
            return false;
        }

        if(is_free && chk_value.length>0){
            var d = dialog({
                title: '信息提示',
                content: '您确定要解锁所选择点位？',
                okValue: '确定',
                ok: function () {
                    $.ajax( {
                        url:'/points/mutile_unlock_point',
                        data: {
                            'ids':chk_value,
                            'chk_code':chk_code
                        },
    ...
    $(".mutile_lock").click(function(){
        ...
        if(!is_free){
            artDialog("信息提示","你选择的点位里含有已经占用或者预定的点位！");
            return false;
        }
        if (chk_value.length == 0) {
            artDialog("信息提示","你选择你要锁定的点位");
            return false;
        }
       if(is_free && chk_value.length>0){
         $("#exampleModal").modal('show');
    ...
    $(".customer-del").click(function(){
        ...
        delShowDialog("删除提示","你确定删除该客户？",failInfo,getUrl,url,id);
    });
