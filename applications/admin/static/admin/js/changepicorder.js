$(function(){
	$(".select2").css('width','251px').select2({allowClear:true});
    ...
    $('.search-point').click(function(){
        if ($("input[name='order_code']").val()) {
            $.post('/changepicorders/get_points', {order_code: $("input[name='order_code']").val(), order_type: $("input[name='order_type']").val()}, function(data){
                if (data.flag && data.points_lists.length > 0) {
                    ...
                    for (var i = 0; i < lists.length; i++) {
                        ...
                        html += '<td class="col-sm-2" class="col-sm-2">' + lists[i]['points_code'] + '</td>';
                        ...
                        if (order_type == 1 || order_type == 2) {
                            html += '<td class="col-sm-2"><input type="text" style="width:91px" value="'+lists[i]['make_num']+'" /></td>';
                        }
    ...
    $("input[name='is_sample']").change(function(){
        if($(this).val() == 0) {
            $("select[name='sample_color']").val("");
            $("select[name='sample_color']").hide(); 
        } else{
            $("select[name='sample_color']").show();
        }
    });
