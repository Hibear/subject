...
layui.define('layer', function(exports){
  ...
  Upload.prototype.init = function(){
    ...
    return elem.each(function(index, item){
      item = $(item);
	  var url = options.url;
	  if("undefined" != item.attr("data-type")){
		  url= url+"&type="+item.attr("data-type");
	  }
     ...
     // var form = '<form target="'+ elemIframe +'" method="'+ (options.method||'post') +'" key="set-mine" enctype="multipart/form-data" action="'+ (options.url||'') +'"></form>';
      var form = '<form target="'+ elemIframe +'" method="'+ (options.method||'post') +'" key="set-mine" enctype="multipart/form-data" action="'+ (url||'') +'"></form>';
