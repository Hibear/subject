...
<div class="home_background">
    <div class="from">
        ...
        <div class="field">
            <span>收货地址：</span><input type="text" id="addr"/>
        </div>
        <div class="sign_in">
            <button style="width: 5rem;height: 2rem; outline: none;" onclick="add()">提交</button>
        </div>
        <span style="font-size: 0.8rem;color: #fff;">注：(您必须提交认证信息才能参与签到赚积分) <br>信息填写错误会影响礼品的发放哦</span>
    </div>
</div>
...
<script type="text/javascript">
    ...
    function add(){
        ...
        $.post('/sign/renzheng', {'realname':realname, 'tel':tel, 'addr':addr}, function(data){
            if(data){
                if(data.code == 1){
                	layer.msg(data.msg);
