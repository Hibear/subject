...
<head>
    ...
    <title>我的礼品</title>
    <link rel="stylesheet" href="<?php echo get_css_js_url('myprize.css', 'www')?>">
...
<div id="list_back">
    ...
    <div class="lists">
        ...
        <!--  </a> -->
            ...
            <p>领取时间:<?php echo date('Y-m-d',strtotime($v['create_time']))?></p>
        </div>
        <?php endif; ?>
        <?php endforeach; ?>
        <?php endif; ?>
             ...
             success: function(data){
                 ...
                 if(data.code == 1){
                     ...
                     for(var i=0;i<score.length;i++){
        						 ...
        						 if(status ==2){
        							 html += '<p>领取时间:'+score[i]['create_time']+'</p>';
            					 }
        						 if(status ==1){
        							 html += '<p>兑换时间:'+score[i]['create_time']+'</p>';

        							 if(score[i]['status']==1){
        								 html += '<p class="rec" data='+score[i]['id']+' style="text-align: center;font-size:20px;">领取</p>';

            						 }

            						 if(score[i]['status']==0){
            							 html += '<p style="text-align: center;font-size:20px;">未发放</p>'; 
                					 }
