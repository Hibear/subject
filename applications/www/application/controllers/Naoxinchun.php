...
class Naoxinchun extends MY_Controller{
    ...
    */
    public function index(){
		...
		$url = C("domain.www.url").'/naoxinchun';
        $this->_init($url);
		
		if(intval(date("H")) > 19){
			header('location:http://h5.wesogou.com/naoxinchun/end');
			exit;
		}
	  
		//自定义分享
		$data['title'] = "金鸡报晓闹新春·送红包"; 
		$data['link'] = C("domain.www.url")."/naoxinchun"; 
		$data['imgUrl'] = C("domain.www.url")."/static/naoxinchun/img/word.png"; 
		$data['desc'] = "金鸡报晓闹新春·送红包"; 
        $data['signPackage'] = $this->share($this->app['app_id'],$this->app['app_secret']);
		
		$this->load->view("naoxinchun/index", $data);
    }
	
	
	public function end() {
		$this->load->view("naoxinchun/end");
	}
    ...
    */
	public function add_user(){
		if(IS_POST){
			...
			if($count >= 500){
				$this->return_json(array("info"=>"今天红包已经发放完毕,明天14点再来!","code"=>100));
			}	   
			...
			if(!$count_user){
					$wetcaht_name = $this->session->userdata($this->session_nick_name);
					 ...
					 //判断openid
					$openid_arr = array();
					if(file_exists('logs/naoxinchunopenid.txt')) {
						$openid_arr = json_decode(file_get_contents("logs/naoxinchunopenid.txt"),true);
					...
					//判断ip
					$ip_arr = array();
					if(file_exists('logs/naoxinchunip.txt')) {
						$ip_arr = json_decode(file_get_contents("logs/naoxinchunip.txt"),true);
					...
					if(intval($res_arr['code']) == 1){
						 ...
						 file_put_contents("logs/naoxinchunopenid.txt",json_encode($openid_arr));
						 
						 array_push($ip_arr,$_SERVER["REMOTE_ADDR"]);
						 file_put_contents("logs/naoxinchunip.txt",json_encode($ip_arr));
