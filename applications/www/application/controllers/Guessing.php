...
class Guessing extends MY_Controller{
    ...
    public function detail(){
        ...
        $this->load->view('guessing/detail', $data);
    ...
    private function renzheng($openid, $active_id){
        $res = $this->Mgame_user->get_one('status', ['openid' => $openid]);
        if($res['status'] != 1){
            $this->session->set_userdata('renzheng_back_url', '/guessing/index?active_id='.$active_id);
            $this->return_json(['code' => -1, 'msg' => '你还未认证个人信息，请先完成认证！']);
        }
    }
    ...
    private function check_today_and_vote( $active_id, $openid, $obj_id, $counts){
        ...
        if($res){
            ...
            if($res){
                $this->return_json(['code' => 0, 'msg' => '竞猜成功！']);
            }else{
    ...
    private function check_login($id = 0){
        $user_info = $this->session->userdata('user_info');
        if(!$user_info){
            $this->session->set_userdata('login_back_url', '/guessing/index?active_id='.$id);
    ...
    public function code($id = 0){
        ...
        $this->valicode->outImg('jc_yzm_'.$id);
    }
}